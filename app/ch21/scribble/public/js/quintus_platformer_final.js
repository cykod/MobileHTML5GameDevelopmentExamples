  ...
  Q.TileLayer = Q.Sprite.extend({
    ...
    checkPoints: function(pos,pts,which) {
      for(var i=0,len=pts.length;i<len;i++) {
        var result = this.checkPoint(pos.x+pts[i][0],pos.y+pts[i][1],which);
        if(result) {
          result.point = pts[i];
          return result;
        }
    ...
    load: function(dataAsset) {
      ...
      this.p.rows = dataAsset.length;
      this.p.cols = dataAsset[0].length;
  ...
  Q.PlatformStage = Q.Stage.extend({
    ...
    collide: function(obj) {
      ...
      if(obj.p.collisionMask & this.collision.p.type) {
        while(col = this._tileCollision(obj,col ? col.start+1 : 0)) {
          if(col) {
