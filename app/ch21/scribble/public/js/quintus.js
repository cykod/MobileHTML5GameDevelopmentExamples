  ...
  Q.Evented = Class.extend({
    ...
    unbind: function(event,target,callback) {
      if(!target) {
        ...
        if(l) {
          for(var len = l.length-1,i=len;i>=0;i--) {
            if(l[i][0] == target) {
 ...
 Q.GameObject = Q.Evented.extend({
  ...
  Q.setup = function(id,options) {
    ...
    Q.wrapper = Q.el
                 ...
                 .parent()
                 .css({ width: Q.el.width(), height:h * 2,
                        padding:0, margin:0,
                        margin: '0 auto' });
    ...
    if(options.maximize) {
      ...
      w = window.innerWidth;
      h = window.innerHeight - (touchDevice ? 0 : 5);

      if((w > 640 || h > 480) && touchDevice) { 
    ...
    Q.wrapper.css({height: Q.el.height(), width: Q.el.width()});
  ...
  Q.load = function(assets,callback,options) {
    ...
    var loadedCallback = function(key,obj) {
      ...
      if(progressCallback) { progressCallback(assetsTotal - assetsRemaining,assetsTotal); }
